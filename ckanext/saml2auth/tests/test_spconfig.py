...
def skip_test_read_metadata_local_config():
...
def skip_test_read_metadata_remote_config():
...
def skip_test_read_metadata_remote_url():
    with pytest.raises(KeyError):
        assert get_config()[u'metadata'][u'local']
...
def skip_test_name_id_policy_format_default_not_set():
...
def skip_test_name_id_policy_format_set_in_config():
...
def skip_test_read_acs_endpoint():
...
def skip_test_one_requested_authn_context():
...
def skip_test_two_requested_authn_context():
...
def skip_test_empty_requested_authn_context():
